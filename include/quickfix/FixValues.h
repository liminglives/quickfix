...
namespace FIX
{
  ...
  const char MsgType_RequestForPosition[] = "URP";
  const char MsgType_PositionReport[] = "AP";
  const char MsgType_RequestForBalance[] = "URB";
  const char MsgType_BalanceReport[] = "UBR";
  const char MsgType_UserLogonRequest[] = "UF001";
  const char MsgType_UserLogonResponse[] = "UF002";
  const char MsgType_UserLogoutRequest[] = "UF003";
  const char MsgType_UserLogoutResponse[] = "UF004";
  const char MsgType_UserChangePassWdRequest[] = "UF005";
  const char MsgType_UserChangePassWdResponse[] = "UF006";
  const char MsgType_PositionStatusRequest[] = "UF007";
  const char MsgType_PositionStatusResponse[] = "UF008";
  const char MsgType_MaxOperationPositionStatusRequest[] = "UF009";
  const char MsgType_MaxOperationPositionStatusResponse[] = "UF010";
  const char MsgType_AllOrdersStatusRequest[] = "UF011";
  const char MsgType_AllOrdersStatusResponse[] = "UF012";
  const char MsgType_SettlementResultStatusRequest[] = "UF013";
  const char MsgType_SettlementResultStatusResponse[] = "UF014";
  const char MsgType_SettlementResultComfirmRequest[] = "UF015";
  const char MsgType_SettlementResultComfirmResponse[] = "UF016";
  const char MsgType_SettlementResultComfirmStatusRequest[] = "UF017";
  const char MsgType_MarginRateStatusRequest[] = "UF018";
  const char MsgType_MarginRateStatusResponse[] = "UF019";
  const char MsgType_CommissionRateStatusRequest[] = "UF020";
  const char MsgType_CommissionRateStatusResponse[] = "UF021";
  const char MsgType_CustomerCapitalStatusRequest[] = "UF022";
  const char MsgType_CustomerCapitalStatusResponse[] = "UF023";
  const char MsgType_AgreementStatusRequest[] = " UF024";
  const char MsgType_AgreementStatusResponse[] = " UF025";
  const char MsgType_MarketDataStatusRequest[] = "UF026";
  const char MsgType_MarketDataStatusResponse[] = "UF027";
  const char MsgType_MarketDataStatusCancelRequest[] = "UF028";
  const char MsgType_MarketDataStatusCancelResponse[] = "UF029";
  const char MsgType_ResponseGapFillResendRequest[] = "UF030";
  const char MsgType_InformationIssueRequest[] = "UF031";
  const char MsgType_InformationIssueResponse[] = "UF032";
  const char MsgType_RequestErrorInformation[] = "UF033";
  const char MsgType_AllocationAck[] = "P";
  ...
  const char PossResend_POSS_RESEND = 'Y';
  const char PossResend_FIRST_SEND = 'N';
  ...
  const char MessageEncoding_ISO_2022_CN[] = "ISO-2022-CN";
  const char MessageEncoding_GBK[] = "GBK";
  const char MessageEncoding_GB2312[] = "GB2312";
  ...
  const int SessionRejectReason_INVALID_MESSAGE_TYPE = 11;
  const int SessionRejectReason_SEVERAL_TIMES_IN_SAME_TAG = 13;
  const int SessionRejectReason_ORDERED_TAG_ORDER_ERROR = 14;
  const int SessionRejectReason_REPEAT_TAG_ORDER_ERROR = 15;
  const int SessionRejectReason_REPEAT_TAG_TIME_ERROR = 16;
  const int SessionRejectReason_NON_DATA_TAG_APPEAR_DEFINE = 17;
  ...
  const char OrdStatus_PENDING_EXPIRE = 'P';
  ...
  const char OrdType_BEST_PRICE = 'a';
  const char OrdType_NEW_PRICE = 'b';
  const char OrdType_NEW_PRICE_UP1_TICKS = 'c';
  const char OrdType_NEW_PRICE_UP2_TICKS = 'd';
  const char OrdType_NEW_PRICE_UP3_TICKS = 'e';
  const char OrdType_Sell1 = 'f';
  const char OrdType_SELL1_UP1_TICKS = 'g';
  const char OrdType_SELL1_UP2_TICKS = 'h';
  const char OrdType_SELL1_UP3_TICKS = 'i';
  const char OrdType_Buy1 = 'j';
  const char OrdType_BUY1_UP1_TICKS = 'k';
  const char OrdType_BUY1_UP2_TICKS = 'l';
  const char OrdType_BUY1_UP3_TICKS = 'm';
  ...
  const char TimeInForce_AGGREGATE_AUCTION = '7';
  ...
  const char OpenClose_CLOSE_YESTERDAY  = 'Y';
  const char OpenClose_CLOSE_TODAY = 'T';
  const char OpenClose_FORCE_CLOSE = 'Q';
  ...
  const char MiscFeeType_TRANSFERFEE[] = "13";
  ...
  const char ExecType_TRADE_PARTIAL_FILL_OR_FILL = 'F';
  ...
  const char SettlLocation_LOCAL_MARKET_SETTLE_LOCATION[] = "ISO";
  ...
  const char SecurityType_MISCELLANEOUS_PASSTHRU[] = "MPT";
  const char SecurityType_CATS_TIGERS[] = "ZOO";
  ...
  const char SecurityExchange_ZHENGZHOU_FUTURES_EXCHANGE = 'ZZCE';
  const char SecurityExchange_DALIAN_FUTURES_EXCHANGE = 'DLCE';
  const char SecurityExchange_HONGKONG_STOCK_EXCHANGE = 'XHKG';
  const char SecurityExchange_SHANGHAI_STOCK_EXCHANGE = 'XSHG';
  const char SecurityExchange_SHENZHEN_STOCK_EXCHANGE = 'XSHE';
  const char SecurityExchange_CHINA_FINANCIAL_FUTURES_EXCHANGE = 'CCFX';
  const char SecurityExchange_CHINA_STAINLESS_STEEL_EXCHANGE = 'CSSX';
  const char SecurityExchange_SHANGHAI_GOLD_EXCHANGE = 'SGEX';
  const char SecurityExchange_CHINA_FOREIGN_EXCHANGE_TRADE_SYSTEM = 'XCFE';
  const char SecurityExchange_DALIAN_COMMODITY_EXCHANGE = 'XDCE';
  const char SecurityExchange_SHANGHAI_FUTURES_EXCHANGE = 'XSGE';
  const char SecurityExchange_SHANGHAI_INTERNATIONAL_ENERGY_EXCHANGE = 'XINE';
  const char SecurityExchange_SHENZHEN_STOCK_EXCHANGE_SHENZHEN_HONG_KONG_STOCK_CONNECT = 'XSEC';
  const char SecurityExchange_SHANGHAI_STOCK_EXCHANGE_SHANGHAI_HONG_KONG_STOCK_CONNECT = 'XSSC';
  const char SecurityExchange_ZHENGZHOU_COMMODITY_EXCHANGE = 'XZCE';
  const char SecurityExchange_STOCK_EXCHANGE_OF_HONG_KONG_LIMITED_SHANGHAI_HONG_KONG_STOCK_CONNECT = 'SHSC';
  const char SecurityExchange_STOCK_EXCHANGE_OF_HONG_KONG_LIMITED_SHENZHEN_HONG_KONG_STOCK_CONNECT = 'SZSC';
  const char SecurityExchange_FDSA_HK = 'HK';
  const char SecurityExchange_FDSA_SH = 'SH';
  const char SecurityExchange_FDSA_SZ = 'ZK';
  const char SecurityExchange_STOCK_EXCHANGE_OF_HONG_KONG_LIMITED_SHANGHAI_HONG_KONG_STOCK_CONNECT_BY_BLOOMBERG = 'HS';
  const char SecurityExchange_STOCK_EXCHANGE_OF_HONG_KONG_LIMITED_SHENZHEN_HONG_KONG_STOCK_CONNECT_BY_BLOOMBERG = 'HZ';
  ...
  const int ExecRestatementReason_MARKET_EXCHANGE_OPTION = 8;
  const int ExecRestatementReason_REPURCHASE_SETTLEMENT = 101;
  ...
  const int PriceType_FIXED_AMOUNT_ABSOLUTE_VALUE = 3;
  ...
  const char CxlRejResponseTo_CANCEL = '1';
  const char CxlRejResponseTo_Replace = '2';
  ...
  const int Style_style1 = 1;
  const int Style_style2 = 2;
  const int Style_style3 = 3;
  const int Style_style4 = 4;
  const int Style_style5 = 5;
  const int RelativePriceLimitBase_RPLNone = 0;
  const int RelativePriceLimitBase_RPLFar = 1;
  const int RelativePriceLimitBase_RPLNear = 2;
  const int RelativePriceLimitBase_RPLBase = 3;
  const int RelativePriceLimitBase_RPLLast = 4;
  const int RelativePriceLimitBase_RPLOpen = 5;
  const int RelativePriceLimitBase_RPLHigh = 6;
  const int RelativePriceLimitBase_RPLLow = 7;
  const int RelativePriceLimitBase_RPLArrival = 8;
  const int RelativePriceLimitBase_RPLAverage = 9;
  const int RelativePriceLimitType_Ticks = 1;
  const int RelativePriceLimitType_Neartouch = 2;
  const int RelativePriceLimitType_Base = 3;
  const int RelativePriceLimitType_TodayOpen = 4;
  const int RelativePriceLimitType_High = 5;
  const int RelativePriceLimitType_Low = 6;
  const int RelativePriceLimitType_Arrival = 7;
  const int RelativePriceLimitType_Average = 8;
  const int StrategyStatus_Initial = 0;
  const int StrategyStatus_Active = 1;
  const int StrategyStatus_Stop = 2;
  const int StrategyStatus_Pause = 3;
  const int StrategyStatus_NotSendIntime = 4;
  const int StrategyStatus_PendingExpire = 8;
  const int StrategyStatus_Rejected = 9;
  const char CloseFlag_CLOSE_YESTERDAY_POSITION = 'Y';
  const char CloseFlag_CLOSE_TODAY_POSITION = 'T';
  const char LogonStatus_LOGON = '0';
  const char LogonStatus_LOGOUT = '1';
  const char LogonStatus_OTHER = '9';
  const char RiskLevel_NORMAL = '0';
  const char RiskLevel_ADD = '1';
  const char RiskLevel_FORCE_CLOSE = '2';
  const char RiskLevel_WARNING = '3';
  const char RiskLevel_DETONATION_POSITIONS = '4';
  const char RiskLevel_EXCEPTION = '5';
  const char ClientSecuType_SAFE_CUSTOMER = '0';
  const char ClientSecuType_LOW_RISK_CUSTOMER = '1';
  const char ClientSecuType_DANGEROUS_CUSTOMER = '2';
  const char ClientSecuType_FREEZEN_CUSTOMER = '3';
  const char ClientSecuType_ATTENTION_CUSTOMER = '4';
  const char HedgeFlag_SPECULATE = '0';
  const char HedgeFlag_HEDGING = '1';
  const char HedgeFlag_ARBITRAGE = '2';
  const char TouchCondition_IMMEDIATELY = '1';
  const char TouchCondition_STOP_LOSS = '2';
  const char TouchCondition_CHECK_WIN = '3';
  const char TouchCondition_EMBEDDED_SINGLE = '4';
  const char TouchCondition_CONDITION_THAN_LATEST = '5';
  const char TouchCondition_CONDITION_EQUAL_GREATER_THAN_LATEST = '6';
  const char TouchCondition_CONDITION_LESS_THAN_LATEST = '7';
  const char TouchCondition_CONDITION_LESS_THAN_EQUAL_LATEST  = '8';
  const char TouchCondition_CONDITION_THAN_SELL1 = '9';
  const char TouchCondition_CONDITION_EQUAL_GREATER_THAN_SELL1 = 'A';
  const char TouchCondition_CONDITION_LESS_THAN_SELL1 = 'B';
  const char TouchCondition_CONDITION_LESS_THAN_EQUAL_SELL1 = 'C';
  const char TouchCondition_CONDITION_THAN_BUY1 = 'D';
  const char TouchCondition_CONDITION_EQUAL_GREATER_THAN_BUY1 = 'E';
  const char TouchCondition_CONDITION_LESS_THAN_BUY1 = 'F';
  const char TouchCondition_CONDITION_LESS_THAN_EQUAL_BUY1 = 'H';
  const char SettlementConfirm_CONFIRM_SETTLEMENT_RESULT = '0';
  const char SettlementConfirmResult_SETTLEMENT_RESULT_HAS_CONFIRM  = '0';
  const char SettlementConfirmResult_SETTLEMENT_RESULT_NOT_CONFIRM = '1';
  const char GapMessageType_POSITION_RESPONSE_GAP = '0';
  const char GapMessageType_ALL_THE_ORDER_STATUS_GAP = '1';
  const char GapMessageType_SETTLEMENT_RESULT_GAP = '2';
  const char GapMessageType_QUERY_CONTRACT_GAP = '3';
  const char ChangePWResult_CHANGE_PASSWORD_SUCCESS = '0';
  const char ChangePWResult_CHANGE_PASSWORD_FAILED = '1';
  const char PassWdType_TRADE_PASSWORD = '0';
  const char PassWdType_FUNDS_PASSWORD = '1';
  const char PassWdType_ORDER_PASSOWRD = '2';
