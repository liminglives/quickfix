...
namespace FIX
{
  ...
  DEFINE_INT(StrategyType);
  DEFINE_STRING(StartTime);
  DEFINE_STRING(EndTime);
  DEFINE_PERCENTAGE(ParticipationRate);
  DEFINE_INT(Style);
  DEFINE_FLOAT(MinVolParticipation);
  DEFINE_FLOAT(RefPrice);
  DEFINE_STRING(OPG);
  DEFINE_STRING(MOC);
  DEFINE_QTY(MinDisplaySize);
  DEFINE_INT(CountEligibleVolumewithinLimitPrice);
  DEFINE_INT(ResetEligibleVolumewithinLimitPrice);
  DEFINE_INT(MaxPriceLevels);
  DEFINE_INT(MaxOrdersPerLevels);
  DEFINE_INT(RelativePriceLimitBase);
  DEFINE_INT(RelativePriceLimitType);
  DEFINE_STRING(RelativePriceLimitOffset);
  DEFINE_FLOAT(AMOpenPercent);
  DEFINE_FLOAT(PMClosePercent);
  DEFINE_FLOAT(AMPercent);
  DEFINE_INT(StrategyStatus);
  DEFINE_INT(PauseType);
  DEFINE_FLOAT(TargetProgress);
  DEFINE_FLOAT(EligibleMarketQty);
  DEFINE_FLOAT(EligibleMarketAmt);
  DEFINE_FLOAT(SerialNum);
  DEFINE_STRING(RegId);
  DEFINE_INT(BatchNum);
  DEFINE_STRING(NotificationReason);
  DEFINE_STRING(VWapMarketPrice);
  DEFINE_STRING(RealParticipationRate);
  DEFINE_FLOAT(ArrivalPrice);
  DEFINE_STRING(UserId);
  DEFINE_INT(QtyMode);
  DEFINE_CHAR(CloseFlag);
  DEFINE_STRING(LogonPasswd);
  DEFINE_CHAR(LogonStatus);
  DEFINE_STRING(AccountName);
  DEFINE_CHAR(RiskLevel);
  DEFINE_AMT(AdditionalMargin);
  DEFINE_CHAR(ClientSecuType);
  DEFINE_STRING(LastLogonIP);
  DEFINE_UTCTIMESTAMP(LastLogonTime);
  DEFINE_CHAR(HedgeFlag);
  DEFINE_CHAR(TouchCondition);
  DEFINE_FLOAT(Riskratio);
  DEFINE_PRICE(LatestPx);
  DEFINE_STRING(InformationID);
  DEFINE_AMT(TotalExMarginAmt);
  DEFINE_QTY(TdPosition);
  DEFINE_QTY(YDPosition);
  DEFINE_QTY(FrozenPosition);
  DEFINE_AMT(FrozenAmt);
  DEFINE_UTCTIMESTAMP(PositionDate);
  DEFINE_AMT(FrozenCommision);
  DEFINE_AMT(PositionProfit);
  DEFINE_AMT(PositionPrice);
  DEFINE_QTY(MaxOpenPosition);
  DEFINE_QTY(MaxClosePosition);
  DEFINE_QTY(MaxCloseTdPosition);
  DEFINE_INT(TotalRetNum);
  DEFINE_INT(PresentRetNum);
  DEFINE_UTCTIMESTAMP(SettlementDate);
  DEFINE_CHAR(SettlementConfirm);
  DEFINE_CHAR(SettlementConfirmResult);
  DEFINE_FLOAT(SpecLongMarginRate);
  DEFINE_FLOAT(SpecShortMarginRate);
  DEFINE_FLOAT(HedgeLongMarginRate);
  DEFINE_FLOAT(HedgeShortMarginRate);
  DEFINE_AMT(SpecLongMarginAmt);
  DEFINE_AMT(SpecShortMarginAmt);
  DEFINE_AMT(HedgeLongMarginAmt);
  DEFINE_AMT(HedgeShortMarginAmt);
  DEFINE_FLOAT(OpenCommissionRate);
  DEFINE_AMT(OpenCommissionAmt);
  DEFINE_FLOAT(CloseCommissionRate);
  DEFINE_AMT(CloseCommissionAmt);
  DEFINE_FLOAT(CloseTdCommissionRate);
  DEFINE_AMT(CloseTdCommissionAmt);
  DEFINE_AMT(BuyMarginAmt);
  DEFINE_AMT(SellMarginAmt);
  DEFINE_AMT(SupplementalMarginAmt);
  DEFINE_AMT(OccupyMarginAmt);
  DEFINE_AMT(TotalMarginAmt);
  DEFINE_PRICE(PreSettlementPrice);
  DEFINE_PRICE(PreClosePrice);
  DEFINE_QTY(PreHoldPosition);
  DEFINE_PRICE(OpenPrice);
  DEFINE_PRICE(HighestPrice);
  DEFINE_PRICE(LowestPrice);
  DEFINE_QTY(HoldPosition);
  DEFINE_PRICE(ClosePrice);
  DEFINE_PRICE(SettlementPrice);
  DEFINE_PRICE(UpperLimitPrice);
  DEFINE_PRICE(LowerLimitPrice);
  DEFINE_QTY(BidQty);
  DEFINE_QTY(AskQty);
  DEFINE_UTCTIMESTAMP(UpdateTime);
  DEFINE_STRING(UpdateMillisec);
  DEFINE_NUMINGROUP(NoOfferPriceLevel);
  DEFINE_NUMINGROUP(NoBidPriceLevel);
  DEFINE_CHAR(GapMessageType);
  DEFINE_INT(GapStartNum);
  DEFINE_INT(GapEndNum);
  DEFINE_CHAR(ChangePWResult);
  DEFINE_STRING(RequestMsgType);
  DEFINE_CHAR(ErrorCode);
  DEFINE_STRING(ExchangeName);
  DEFINE_AMT(TotalFrozenAmt);
  DEFINE_QTY(OneLotQty);
  DEFINE_QTY(MaxLotQty);
  DEFINE_QTY(MaxHoldPosition);
  DEFINE_STRING(VarietyCode);
  DEFINE_STRING(VarietyName);
  DEFINE_AMT(MinPxAlterUnit);
  DEFINE_AMT(YesterdayStlAmt);
  DEFINE_AMT(BuyFrozenAmt);
  DEFINE_AMT(SellFrozenAmt);
  DEFINE_AMT(UseableAmt);
  DEFINE_AMT(FloatProfit);
  DEFINE_AMT(CloseProfit);
  DEFINE_AMT(DayFolatProfit);
  DEFINE_STRING(RequestID);
  DEFINE_CHAR(PassWdType);
  DEFINE_STRING(OldPassWd);
  DEFINE_STRING(NewPassWd);
  DEFINE_AMT(SettleFee);
  DEFINE_STRING(DeclarationID);
  DEFINE_STRING(TradeID);
  DEFINE_BOOLEAN(NextFlag);
  DEFINE_STRING(IPAddress);
  DEFINE_STRING(MacAddress);
  DEFINE_AMT(FetchAmt);
  DEFINE_AMT(DayPaymentAmt);
  DEFINE_AMT(DayIncomeAmt);
  DEFINE_UTCTIMESTAMP(OrderEntryTime);
